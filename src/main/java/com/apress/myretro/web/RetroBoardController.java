...
public class RetroBoardController {
    ...
    public ResponseEntity<RetroBoard> saveRetroBoard(@Valid @RequestBody RetroBoard retroBoard){
        ...
        URI location = ServletUriComponentsBuilder.
                fromCurrentRequest().path("/{uuid}").buildAndExpand(result.getId().toString()).toUri();
    ...
    public ResponseEntity<Card> addCardToRetroBoard(@PathVariable UUID uuid, @Valid @RequestBody Card card){
        Card result = retroBoardService.addCardToRetroBoard(uuid,card); //here uuid of RetroBoard
        URI location = ServletUriComponentsBuilder.
                fromCurrentRequest().path("/{uuid}/cards/{uuidCard}").buildAndExpand(result.getId().toString()).toUri();
        return ResponseEntity.created(location).body(result);
    }
    ...
    public Map<String, Object> handleValidationExceptions( MethodArgumentNotValidException ex) {
        ...
        response.put("time", LocalDateTime.now().format(DateTimeFormatter.
